...
helyettesített. Ezek az ún.\ klasszikus rejtjelek helyettesítésen vagy transzpozíción alapultak, és
...
entrópia, a tökéletes titkosság és a biztonság formális fogalmait~\cite{shannon1949}. Shannon
...
A 2000-es évektől a kriptográfia az internet mindennapi infrastruktúrájává vált. A TLS (Transport
Layer Security) protokoll révén az RSA és az ECC az HTTPS, a biztonságos e-mail és a VPN
alapszöveteként működik. A nyilvános kulcsú infrastruktúra (PKI) tanúsítványok millióit gondozza,
és az internet kommunikáció biztonságának gerincét alkotja. Ez a globális, komplex infrastruktúra
áll most szemben a kvantumszámítástechnika által felvetett kihívással.
...
szabályai írják le. A qubit állapotát egy kétdimenziós komplex Hilbert-tér elemeként ábrázoljuk:
...
jelenség az összefonódás (entanglement): két qubit összefonódott állapotban olyan módon korrelált,
...
akkumulátormateriak tervezése) és a katalízisvizsgálatban.
...
és menetrend-tervezés területén már kisebb méretű kísérletekben is demonstráltak kvantum
előnyt.
...
\textbf{A Shor-algoritmus} 1994-ben Peter Shor bebizonyította, hogy egy kvantumszámítógép polinomiális
...
$O(\exp((\log N)^{1/3}))$ nagyságrendű. Ez közvetlenül aláása az RSA, a Diffie--Hellman
kulcscsere és az ECC biztonságát: az RSA-2048 feltöréséhez hozzávetőleg 4\,000 hibajavított
logikai qubit szükséges --- ez a szám a jelenlegi fejlesztési ütem alapján a 2030-as évekre
lehet elérhető.

\textbf{A Grover-algoritmus} 1996-ban Lov Grover megmutatta, hogy egy $N$ elemű rendezetlen
adatbázisban a keresés kvantumszámítógépen $O(\sqrt{N})$ lépésben elvégezhető~\cite{grover1996},
...
láthatatlanul végzik munkájukat~\cite{weiser1991}. Az 1990-es évek elején a Carnegie Mellon
...
követhetővé válik~\cite{ashton1999}. Az RFID technológia lehetővé tette például a raktári
...
egy támadó a klórszint módosításával próbálkozott, jól illusztrálta az IIoT rendszerek fizikai
...
megfelelő zajellenintézkedéseket.
...
\begin{itemize}
    ...
    \item \textbf{Izogeníán alapuló kriptográfia:} a SIKE algoritmust 2022-ben klasszikus
...
A dolgozat fő kutatási kérdése: \textit{Hogyan alkalmazhatók a CRYSTALS-Kyber és BIKE
posztkvantum-kriptográfiai algoritmusok erőforrás-korlátozott IoT eszközökön, és milyen
teljesítménybeli különbségek mutatkoznak köztük valós mérési körülmények között?}
...
\begin{enumerate}
    ...
    \item Mérési környezet kialakítása, amely lehetővé teszi a két algoritmus
          valós erőforrás-korlátozott eszközön való teljesítmény-összehasonlítását.
...
A dolgozat felépítése a következő. A \textbf{2.\ fejezet} részletesen tárgyalja a
posztkvantum-kriptográfiai protokollok matematikai alapjait, a CRYSTALS-Kyber és a BIKE
algoritmusok működési mechanizmusát és biztonsági tulajdonságait. A \textbf{3.\ fejezet}
bemutatja a mérési környezetet: az alkalmazott hardvert, szoftverkörnyezetet és a wolfSSL
kriptográfiai könyvtár PQC-implementációját. A \textbf{4.\ fejezet} leírja a mérési
módszertant, a mért paramétereket és a mérési eljárást. Az \textbf{5.\ fejezet} tartalmazza
a mérési eredményeket és azok részletes kiértékelését táblázatos és grafikus formában.
A \textbf{6.\ fejezet} összefoglalja a következtetéseket, és javaslatokat fogalmaz meg a
PQC IoT rendszerekben való alkalmazásának jövőbeli irányait illetően.
